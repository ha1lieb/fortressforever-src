...
void CFFPlayer::ScoutRadarThink(void)
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if (pPlayer && (pPlayer != this))
		{
			...
			if (!bIsSpy && pPlayer->GetAbsVelocity().LengthSqr() < (RADAR_MINSPEED * RADAR_MINSPEED))
			continue;
	...
	Class_T radarBuildableClasses[] = { CLASS_SENTRYGUN, CLASS_DISPENSER, CLASS_DETPACK, CLASS_MANCANNON };
	...
	CBaseEntity* pFlagEnt = gEntList.FindEntityByClassT(NULL, CLASS_INFOSCRIPT);
	while (pFlagEnt)
	{
